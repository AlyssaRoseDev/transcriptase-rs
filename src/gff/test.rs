...
use crate::err::{TXaseResult, TXaseError};
...
fn full() -> TXaseResult<()> {
    ...
    let mut src = String::with_capacity(file.metadata()?.len().try_into().map_err(|err: TryFromIntError| TXaseError::InternalParseFailure(err.to_string()))?);
